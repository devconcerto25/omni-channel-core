...
@Component
public class ISO8583RefundHandler implements OperationHandler {
    private static final Logger logger = LoggerFactory.getLogger(ISO8583RefundHandler.class);
    @Autowired
...
//            String responsePayload = connector.processAsync(request).get();
    ...
    public CompletableFuture<TransactionResponse> handleAsync(TransactionRequest request) {
        logger.info("Processing ISO8583 logon request asynchronously");
        ...
        return CompletableFuture
                .supplyAsync(() -> {
                    try {
                        ...
                        return new LogonProcessingContext(connector, requestPayload, request);
                    } catch (Exception e) {
                        throw new RuntimeException("Failed to initialize logon processing context", e);
                    }
                })
                .thenCompose(context -> processLogonWithConnectorAsync(context))
                .exceptionally(throwable -> {
                    logger.error("Error processing ISO8583 logon asynchronously", throwable);
    ...
    private CompletableFuture<TransactionResponse> processLogonWithConnectorAsync(LogonProcessingContext context) {
        return CompletableFuture
                .supplyAsync(() -> {
                    try {
                        ...
                        throw new RuntimeException("Connector processing failed for logon", e);
                    }
                })
                .thenApply(responsePayload -> {
                    try {
                        ...
                        logger.error("Error parsing logon connector response", e);
    ...
    private TransactionResponse createSuccessResponse(TransactionRequest request, String responsePayload, Map<String, Object> responseMap) {
        ...
        if (success) {
            // Extract success data - logon specific fields
            ...
            // Logon specific additional data
            response.addAdditionalData("terminalId", responseMap.get("terminalId"));
            response.addAdditionalData("merchantId", responseMap.get("merchantId"));
            response.addAdditionalData("sessionKey", responseMap.get("sessionKey"));

            logger.info("ISO8583 logon completed successfully");
        } else {
            // Extract error information
            response.setErrorCode((String) responseMap.get("errorCode"));
            response.setErrorMessage((String) responseMap.get("errorMessage"));

            logger.info("ISO8583 logon completed with failure");
        }
    ...
    private TransactionResponse createErrorResponse(TransactionRequest request, Exception e) {
        ...
        errorResponse.setErrorMessage("Logon processing failed: " + e.getMessage());
        errorResponse.setErrorCode("LOGON_ERROR");

        return errorResponse;
    }

    // Helper class to carry context through the async chain
    private static class LogonProcessingContext {
        final Connector connector;
        final String requestPayload;
        final TransactionRequest request;

        LogonProcessingContext(Connector connector, String requestPayload, TransactionRequest request) {
    ...
    public boolean supports(String channel, String operation) {
        return ("ISO8583".equalsIgnoreCase(channel) ||
                ...
                "refund".equalsIgnoreCase(operation);
    }
}
